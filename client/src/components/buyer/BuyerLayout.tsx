...
export function BuyerLayout({ children, breadcrumbs = [] }: BuyerLayoutProps) {
  const [, setLocation] = useLocation();
  const { logout } = useAuth();
  const { hasAccess, isLoading } = useRequireAuth(["buyer", "admin"]);
  ...
  return (
    <SidebarProvider style={sidebarStyle}>
      <div className="flex min-h-screen w-full">
        <BuyerSidebar onLogout={handleLogout} />
          ...
          <header className="flex h-14 shrink-0 items-center gap-2 border-b px-4">
            ...
            {breadcrumbs.length > 0 && (
              <Breadcrumb>
                <BreadcrumbList>
                  <BreadcrumbItem>
                    <BreadcrumbLink href="/buyer" data-testid="breadcrumb-dashboard">
                      Dashboard
                    </BreadcrumbLink>
                  </BreadcrumbItem>
